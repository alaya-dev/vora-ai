<x-admin-layout>
    <!-- Header/Title -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('dashboard.title') }}</h1>
        <p class="mt-1.5 text-sm text-gray-500">{{ __('dashboard.welcome') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="relative overflow-hidden bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('dashboard.stats_total_users') }}</p>
        ...
        <div class="relative overflow-hidden bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('dashboard.stats_admins') }}</p>
        ...
        <div class="relative overflow-hidden bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 group">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('dashboard.stats_normal_users') }}</p>
    ...
    <div class="bg-white border border-gray-200/80 rounded-2xl p-8 shadow-sm">
        <div class="max-w-2xl">
            ...
            <p class="text-gray-500 leading-relaxed mb-6">
                {{ __('dashboard.logged_in_admin') }} {{ __('dashboard.admin_desc') }}
            </p>
