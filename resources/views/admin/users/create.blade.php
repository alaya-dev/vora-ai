<x-admin-layout>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('users.add') }}</h1>
        <p class="mt-1.5 text-sm text-gray-500">{{ __('users.form.create_desc') }}</p>
    </div>
    ...
    <div class="max-w-xl bg-white border border-gray-200/80 rounded-2xl p-8 shadow-sm">
        <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
            ...
            <!-- Password Field -->
            <div>
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('users.form.password') }}</label>
            ...
            <div>
                ...
                <select name="role" id="role" required
                    ...
                    <option value="user" {{ old('role') === 'user' ? 'selected' : '' }}>{{ __('users.form.user') }}</option>
                    <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>{{ __('users.form.admin') }}</option>
                </select>
                @error('role')
                    <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                @enderror
