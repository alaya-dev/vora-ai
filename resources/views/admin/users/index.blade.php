<x-admin-layout>
    <div x-data="{ showDeleteModal: false, deleteAction: '', userName: '' }">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('users.title') }}</h1>
        ...
        <div class="bg-white border border-gray-200/80 rounded-2xl p-5 mb-6 shadow-sm">
            <form action="{{ route('admin.users.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                <div class="relative flex-grow">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="{{ __('users.search') }}"
                ...
                <table class="w-full text-start border-collapse">
                    <thead>
                        <tr class="bg-gray-50/75 border-b border-gray-200/80">
                            ...
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('users.name') }}</th>
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('users.email') }}</th>
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('users.role') }}</th>
                            <th class="px-6 py-4 text-start text-xs font-semibold text-gray-400 uppercase tracking-wider">{{ __('users.created_at') }}</th>
                            <th class="px-6 py-4 text-center text-xs font-semibold text-gray-400 uppercase tracking-wider w-36">{{ __('users.actions') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/40 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex items-center justify-center space-x-2.5 rtl:space-x-reverse">
                                        ...
                                        @else
                                            <span class="p-1.5 text-gray-200 cursor-not-allowed" title="{{ __('alerts.self_delete') }}">
                                                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                </svg>
                                            </span>
        ...
        <div x-show="showDeleteModal" 
            ...
            <div class="bg-white border border-gray-200/80 rounded-2xl overflow-hidden shadow-2xl max-w-md w-full z-10 p-6 transform transition-all"
                ...
                <div class="flex items-start space-x-4 rtl:space-x-reverse">
                    ...
                    <div class="flex-1">
                        <h3 class="text-lg font-bold text-gray-900 leading-6">{{ __('users.delete') }}</h3>
