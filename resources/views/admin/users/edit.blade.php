<x-admin-layout>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-bold text-gray-900 tracking-tight">{{ __('messages.users.edit') }}</h1>
        <p class="mt-1.5 text-sm text-gray-500">{{ __('messages.users.form.edit_desc') }}</p>
    </div>
    ...
    <div class="max-w-xl bg-white border border-gray-200/80 rounded-2xl p-8 shadow-sm">
        <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
            ...
            <!-- Email Field -->
            <div>
                <label for="email" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('messages.users.form.email') }}</label>
            ...
            <div>
                <label for="password" class="block text-sm font-semibold text-gray-700 mb-2">
                    {{ __('messages.users.form.password') }} <span class="text-xs text-gray-400 font-normal">({{ __('messages.users.form.password_help') }})</span>
                </label>
            ...
            <!-- Role Field -->
            <div>
                <label for="role" class="block text-sm font-semibold text-gray-700 mb-2">{{ __('messages.users.form.role') }}</label>
                <select name="role" id="role" required
                    ...
                    <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>{{ __('messages.users.form.user') }}</option>
                    <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>{{ __('messages.users.form.admin') }}</option>
                </select>
                @error('role')
                    <p class="mt-1.5 text-xs text-rose-600 font-medium">{{ $message }}</p>
                @enderror
