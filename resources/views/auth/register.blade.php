<x-guest-layout>
    <!-- Page Heading -->
    <div class="mb-6">
        <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">{{ __('auth_ui.create_account') }}</h2>
        <p class="mt-1.5 text-sm text-slate-500">{{ __('landing.create_account_desc') }}</p>
    </div>

    <!-- Tabs -->
    <div class="flex border border-slate-100 mb-6 p-1 bg-slate-50 rounded-xl">
        <a href="{{ route('login') }}" class="flex-1 text-center py-2 text-xs sm:text-sm font-bold rounded-lg text-slate-500 hover:text-slate-900 transition duration-150">
            {{ __('auth_ui.connexion') }}
        </a>
        <a href="{{ route('register') }}" class="flex-1 text-center py-2 text-xs sm:text-sm font-bold rounded-lg bg-white text-slate-900 shadow-sm transition duration-150">
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <!-- Name -->
        <div>
            <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">{{ __('auth_ui.full_name') }}</label>
            <div class="relative">
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name"
                    ...
                    class="w-full ps-11 pe-4 py-2.5 bg-slate-50 border @error('name') border-rose-300 focus:ring-rose-500/20 focus:border-rose-500 @else border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-600 @enderror rounded-xl focus:bg-white focus:ring-4 transition duration-150 text-sm">
            </div>
            @error('name')
        ...
        <!-- Confirm Password -->
        <div x-data="{ showPassword: false }">
            <label for="password_confirmation" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">{{ __('Confirm Password') }}</label>
            <div class="relative">
                <div class="absolute inset-y-0 start-0 ps-3.5 flex items-center pointer-events-none text-slate-400">
                    <!-- Lock Icon -->
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <input id="password_confirmation" :type="showPassword ? 'text' : 'password'" name="password_confirmation" required autocomplete="new-password"
                    placeholder="••••••••"
                    class="w-full ps-11 pe-12 py-2.5 bg-slate-50 border @error('password_confirmation') border-rose-300 focus:ring-rose-500/20 focus:border-rose-500 @else border-slate-200 focus:ring-emerald-500/20 focus:border-emerald-600 @enderror rounded-xl focus:bg-white focus:ring-4 transition duration-150 text-sm">
        ...
        <div class="pt-2">
            <button type="submit" class="w-full flex items-center justify-center px-5 py-3 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-emerald-500/10 hover:shadow-emerald-500/20 transition duration-150">
                {{ __('auth_ui.create_secure_account') }}
            </button>
        </div>
            ...
            <div class="flex items-center justify-center gap-3">
                <button type="button" class="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 transition text-sm font-semibold">
                    ...
                    <svg class="w-4 h-4" viewBox="0 0 24 24">
                        <path fill="#EA4335" d="M12.24 10.285V14.4h6.887c-.275 1.565-1.88 4.604-6.887 4.604-4.33 0-7.866-3.577-7.866-8s3.536-8 7.866-8c2.46 0 4.105 1.025 5.047 1.926l3.245-3.125C18.465 2.021 15.65 1 12.24 1 6.033 1 12.24s5.033 11.24 11.24 11.24c6.478 0 10.793-4.537 10.793-10.986 0-.74-.08-1.305-.177-1.785H12.24z"/>
                    </svg>
                    Google
                </button>
                <button type="button" class="flex-1 flex items-center justify-center gap-2 px-4 py-2 border border-slate-200 rounded-xl text-slate-600 hover:bg-slate-50 transition text-sm font-semibold">
        ...
        <div class="text-center pt-4 border-t border-slate-100 flex flex-col items-center gap-3">
            <p class="text-sm text-slate-500">
                {{ __('auth_ui.already_registered_question') }} 
                <a href="{{ route('login') }}" class="font-bold text-emerald-600 hover:text-emerald-700 transition duration-150">
                    {{ __('Login') }}
                </a>
            </p>
