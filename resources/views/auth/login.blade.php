<x-guest-layout>
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-900 tracking-tight">{{ __('auth.login') }}</h2>
        <p class="mt-1 text-sm text-gray-500">{{ __('auth.welcome_back') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                <label for="password" class="block text-sm font-semibold text-gray-700">{{ __('auth.password') }}</label>
        ...
        <div class="flex items-center">
            ...
            <label for="remember_me" class="ms-2 text-sm font-medium text-gray-600 select-none">{{ __('auth.remember_me') }}</label>
        </div>
